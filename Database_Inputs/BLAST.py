...
Will run BLAST against the file pointed to when this function is called on the commandline
...
The quality threshold used on our BLAST results (pident and length/slength) are to
be set via the commandline flag -d (which has its values sent in via processor.py)

NOTE: ENSURE THAT ALL FASTAFILE NAMES HAVE NO SPACES NOR SPECIAL CHARACTERS IN THEM
"""
import os
import subprocess
...
parser.add_argument("--data", "-d", help="The user entered pident, followed by Len/Slen values to be used in the BLAST call (the values are to be separated by a '~'")
args = parser.parse_args()
URL = args.URL

PIDENT = float(args.data.split("~")[0]) #extracting the user set pident and len/slen values
LENSLEN = float(args.data.split("~")[1])
...
for database in databases:
    ...
    if counter > 0: #I only have the BoTN database to play with right now. Take out when FlaA&B databases are added
        continue
    ...
    cmdln = "blastn -query " + URL + " -db " + databases[counter] + " -outfmt '6 qseqid stitle qlen slen length pident mismatch gaps qstart qend qseq sstrand' > " + out_dirs[counter] + file_name + ".blast.tsv"
    ...
    if x ==0:
        ...
        if os.path.isfile(extended_fastas[counter] + file_name + ".blast.fasta"): #cleaning up old temp files
            os.remove(extended_fastas[counter] + file_name + ".blast.fasta")
            
    else: #very ugly but it seems to be the only solution to the wierdest problem ever
        if os.path.isfile(extended_fastas[counter] + file_name + ".no_hits"): #cleaning up old temp files
            os.remove(extended_fastas[counter] + file_name + ".no_hits")
        ...
        for line in blast_file: 
            ...
            if (length/slen) > LENSLEN and pident >= PIDENT: #if the line's metadata sucks, then we toss everything about that line into the abyss
                ...
                counter117 = 0 #counter which has definitly not been used before, used to track how many lines we have compared against
                if num_to_compare > 0:
                    for compare_line in lines_we_care_about: #compare the newly cleared line against all other cleared lines to avoid douplicates with the same indexes
                        counter117 +=1
                        if line is compare_line: #quick check for duplicates
                            break
                        if is_overlapping(line, compare_line):
                            ...
                            break #if our line beat another, then we are done here and we are ready to move on
                            
                        if counter117 == num_to_compare: #if our line makes it through the long haul (of comparing against  all the other approved lines), add it in
        ...
        for line in lines_we_care_about:
            ...
            run = False #if run is set ture, we will start recording the contig to a string
            full_contig = ""
        ...
        if os.path.getsize(extended_fastas[counter] + file_name + ".blast.fasta") == 0: #if the output file is empty
            os.rename(extended_fastas[counter] + file_name + ".blast.fasta", extended_fastas[counter] + file_name + ".no_hits") #give it the .no_hits name instead
